...
source = zeros([size(c,1),1]);
source(2531:2561)= 1;

a = (2531-2561+1)*squaresize;
...
figure()
sgtitle("Diffraction - single slit - 1D approach")
r = c(1,:,1);
...
% RS propagation pattern
    ...
    subplot 262
    imagesc(0.5,c(1,:,1),abs(propagatedRS1));
    title("RS propagation - impulse response");xlabel("[px]");ylabel("[m]");

    % RS transfer function
    propagatedRS_resc = rescale_interpol1D(propagatedRS2(:,round(gridsize(1)/2)+1),RSX,gridsize,c);
    subplot 263
    imagesc(0.5,c(1,:,1),abs(propagatedRS_resc))
    title("RS propagation - transfer function ");xlabel("[px]");ylabel("[m]");
...
% RS pattern validation 
    ...
    subplot 268
    hold on
    scatter(abs(propagatedRS1),r,10,DisplayName='Simulated amplitude profile',  MarkerEdgeColor= "#D95319")
    yline(0, LineStyle="-.");
    yline(m1, LineStyle="-.");yline(-m1, LineStyle="-.");
    xlabel("Amplitude"); ylabel("[px]");
    title("Rayleigh-Sommerfeld propagation")

    % transfer function validation
    subplot 269
    hold on
    scatter(abs(propagatedRS_resc),r,10,MarkerEdgeColor = "#D95319")
    yline(0, LineStyle="-.");
    yline(m1, LineStyle="-.");yline(-m1, LineStyle="-.");
    xlabel("Amplitude"); ylabel("[px]");
    title("Rayleigh-Sommerfeld propagation")
...
% Fresnel pattern propagation
    ...
    subplot 264
    imagesc(0.5,c(1,:,1),abs(propagatedFresnel1))
    title("FR propagation - impulse response");xlabel("[px]");ylabel("[m]");

    % Fresnel quadratic factor
    propagatedFr_resc = rescale_interpol1D(propagatedFresnel2(:,round(gridsize(1)/2)+1),FRX,gridsize,c);

    subplot 265
    imagesc(0.5,c(1,:,1),abs(propagatedFr_resc))
    title("FR propagation - quadratic factor");xlabel("[px]");ylabel("[m]");

% Fresnel pattern validation
    % impulse response
    subplot(2,6,10)
    hold on
    scatter(abs(propagatedFresnel1),r,10,  MarkerEdgeColor = "#D95319")
    yline(0, LineStyle="-.");
    yline(m1, LineStyle="-.");yline(-m1, LineStyle="-.");
    xlabel("Amplitude"); ylabel("[px]");
    ...
    subplot(2,6,11)
    hold on
    scatter(abs(propagatedFr_resc),r,10,MarkerEdgeColor = "#D95319")
    yline(0, LineStyle="-.");
    yline(m1, LineStyle="-.");yline(-m1, LineStyle="-.");
    xlabel("Amplitude"); ylabel("[px]");
    title("Fresnel propagation");
...
% Fraunhoffer pattern
    propagatedFraun_resc = rescale_interpol1D(propagatedFraun(:,round(gridsize(1)/2)+1),FFX,gridsize,c);

    subplot 266
    imagesc(0.5,c(1,:,1),abs(propagatedFraun_resc))
    title("Fraunhofer propagation");xlabel("[px]");ylabel("[m]");

% Fraunhoffer pattern validation
    subplot(2,6,12)
    hold on
    scatter(abs(propagatedFraun_resc),r,10, MarkerEdgeColor = "#D95319")
    yline(0, LineStyle="-.");
    yline(m1, LineStyle="-.");yline(-m1, LineStyle="-.");
    xlabel("Amplitude"); ylabel("[px]");
